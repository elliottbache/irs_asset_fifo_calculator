...
class TestParseBuyAndSell:
    ...
    "rows, fee_rows, is_buy, expected_asset, expected_amount, expected_price, expected_total, expected_error, expected_error_message", [
            (
                [
            ...
            (
                [
                    ...
                    make_row('NVDA', 10, buy=125.0, tx_type='Exchange'),
                    make_row('feeUSD', -10.0, sell=1.0, tx_type='Exchange'),
                ], [2], True, 'NVDA', 10.0, 125.0, 1260.0, ValueError, "Multiple rows for buy or sell must be implemented"
            ),
            (
                [
            ...
            (
                ...
                ], [2], True, 'NVDA', 9.9, 125.0, 1262.5, ValueError, "Fee price does not match buy or sell price for"
            ),

        ],
        ids=['exchange_buy', 'exchange_sell', 'usd_buy', 'transfer', 'usd_sell',
             'buy_with_same_asset_fees', 'sell_with_same_asset_fees', 'two_buys',
             'missing_no_buy_or_sale_non_transfer', 'buy_with_same_asset_fee_wrong_price']
    )

    def test_parse_buy_and_sell(self, rows, fee_rows, is_buy, expected_asset,
                                expected_amount, expected_price, expected_total, expected_error,
        ...
        if expected_error is None:
            asset, amount, price, total = calculate_taxes.parse_buy_and_sell(is_buy, block_type, rows, fee_assets, fee_rows)
            ...
            with pytest.raises(expected_error, match=expected_error_message):
                calculate_taxes.parse_buy_and_sell(is_buy, block_type, rows, fee_assets, fee_rows)
...
class TestParseRowData:
    ...
    "fee_asset, amount_fee_asset, price_fee_asset, buy_asset, buy_amount, buy_price, expected", [
        ...
        ids=['buy_fee_asset_same_as_sell', 'buy_fee_same_as_sell', 'buy_fee_exceeds_sell',
             'buy_fee_asset_same_as_buy', 'buy_fee_same_as_buy', 'buy_fee_exceeds_buy',
             'buy_different_fee_asset'
             ]
    )

    def test_parse_row_data_buy(self, fee_asset, amount_fee_asset, price_fee_asset, buy_asset, buy_amount, buy_price, expected,
                                     rows):
        block_type = 'Buy'
        rows = rows.drop(0)
        rows = rows.reset_index(drop=True)
    ...
    "fee_asset, amount_fee_asset, price_fee_asset, expected", [
        ...
        ids=['sell_same_fee_asset_as_buy', 'sell_fee_exceeds_buy', 'sell_same_fee_asset_as_sell', 'sell_fee_exceeds_sell'
             ]
    )

    def test_parse_row_data_sell(self, fee_asset, amount_fee_asset, price_fee_asset, expected, rows):
        block_type = 'Sell'
        rows = rows.drop(0)
        rows = rows.reset_index(drop=True)
        rows.loc[2, 'Asset'] = fee_asset
...
class TestUpdateFifo:
    ...
    @pytest.mark.parametrize(
        ...
        "buy_data, sell_data, fee_data, expected_behavior, expected_error, expected_error_message",
        [
            (AD('TSLA', 25.0, 49.0, 1235.0, date(2024, 9, 4)),
             AD('NVDA', -25.0 * 49.0 / 120.0, 120.0, 1215.0, date(2024, 9, 4)),
             AD('USD', -10.0, 1.0, 10.0, date(2024, 9, 4)), 'no_change',
    ...
    def test_update_fifo_fee_branch(self, form8949, fifo, buy_data, sell_data, fee_data,
                                    expected_behavior, expected_error, expected_error_message):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ], 1, {"Description": "9.00000000 NVDA", "Date Acquired": "09/04/2024",
            ...
            (
                ...
                ], 1, {"Description": "0.10000000 NVDA", "Date Acquired": "09/04/2024",
            ...
            (
                [
                    ...
                    make_row('feeNVDA', -0.1, sell=125.0),
                    make_row('feeNVDA', -0.06, sell=123.0, tx_type='Exchange', tx_idx=1),
                    make_row('NVDA', -11.95121951, sell=123.0, tx_type='Exchange', tx_idx=1),
                    make_row('TSLA', 20.0, buy=49.0, tx_type='Exchange', tx_idx=1),
                    make_row('feeNVDA', -0.041, sell=123.0, tx_type='Exchange', tx_idx=1),
                ], 1, {"Description": "12.05221951 NVDA", "Date Acquired": "09/04/2024",
            ...
            (
                [
                    ...
                    make_row('TSLA', 20.0, buy=49.0, tx_type='Exchange', tx_idx=1),
                    make_row('feeNVDA', -0.041, sell=123.0, tx_type='Exchange', tx_idx=1),
                ], 1, {"Description": "11.99221951 NVDA", "Date Acquired": "09/04/2024",
